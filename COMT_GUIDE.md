...
- ✅ **图片缓存**: 自动缓存下载的 CoMT 图片到 `~/.cache/redlens/comt_images/`
...
python request.py --mode comt_vsp --comt_sample_id deletion-0107 --max_tasks 5
```
...
✅ 成功加载 3853 条CoMT数据
✅ 缓存目录: ~/.cache/redlens/comt_images/
...
# 首次运行会自动下载 CoMT 数据集
python request.py --mode comt_vsp --max_tasks 1
```
...
```bash
# 实验组 1: 标准 VSP
python request.py --mode vsp --max_tasks 50

# 实验组 2: CoMT-VSP
python request.py --mode comt_vsp --max_tasks 50
...
```bash
# 查看缓存目录
ls -lh ~/.cache/redlens/comt_images/
...
```bash
--mode comt_vsp                        # 使用 CoMT-VSP 执行模式
...
# 清除缓存重试
rm -rf ~/.cache/redlens/comt_images/
python request.py --mode comt_vsp --max_tasks 1
...
**问题**: `~/.cache/redlens/comt_images/` 占用大量空间

**解决方案**:
```bash
# 查看缓存大小
du -sh ~/.cache/redlens/comt_images/

# 清除缓存
rm -rf ~/.cache/redlens/comt_images/

# 下次运行会重新下载
python request.py --mode comt_vsp --max_tasks 5
```
...
# 指定固定 CoMT 样本
python request.py --mode comt_vsp --comt_sample_id creation-10003 --max_tasks 5

# 单个类别深度测试
python request.py --mode comt_vsp --categories 12-Health_Consultation --max_tasks 50

# 完整评估流水线
python request.py --mode comt_vsp --eval_model "gpt-5-mini"

# 对比实验
python request.py --mode vsp --max_tasks 100          # 标准 VSP
python request.py --mode comt_vsp --max_tasks 100     # CoMT-VSP
...
**文档版本**: 3.1
**最后更新**: 2026-02-21
**状态**: ✅ 已完成并测试通过（顺序双任务模式，`--mode comt_vsp`）
